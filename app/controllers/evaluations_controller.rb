class EvaluationsController < ApplicationController
  ...
  before_action(only: %i[current show table]) { authorize_evaluation(:for_view) }
  before_action(except: %i[current show table]) { authorize_evaluation(:for_edit) }
  ...
  def authorize_evaluation(action_type)
    authorize Evaluation.find_or_initialize_by(group: Group.find(params[:group_id])), action_type
  end
end
